...
// Ensure these paths are correct relative to where webhook.js is located
...
// Note: Admin and NominationCategory models are not directly updated by webhooks,
// so they are not strictly necessary to import *just* for the webhook handler,
// but they might be needed elsewhere in the same router file.
...
router.post('/webhook/paystack', async (req, res) => {
  ...
  console.log(
    `Paystack Webhook: Received event type "${event.event}" for reference "${event.data.reference}"`
  )
  ...
  if (event.event === 'charge.success') {
    ...
    try {
      ...
      if (reference && reference.startsWith('vote-')) {
        ...
        console.log(
          'Paystack Webhook: Processing successful vote payment for reference:',
          reference
        )
        ...
        if (!vote) {
          console.error(
            'Paystack Webhook: Vote not found for reference:',
            reference
          )
        ...
        if (vote.payment_status === 'completed') {
          console.log(
            'Paystack Webhook: Vote already completed for reference:',
            reference
          )
        ...
        await vote.save()
        console.log(
          'Paystack Webhook: Marked vote as completed:',
          reference
        )
        ...
        if (!nominee) {
          console.error(
            'Paystack Webhook: Nominee not found for vote:',
            vote._id
          )
          ...
          console.log(
            `Paystack Webhook: Updated nominee "${nominee.name}" vote count: +${vote.number_of_votes}. New total: ${nominee.number_of_votes}`
          )
        }
      } else if (
        reference &&
        ...
        console.log(
          'Paystack Webhook: Processing successful ticket payment for reference:',
          reference
        )
        // Extract ticket ID from the reference (e.g., 'ticket-60c72b2f9b1d4b001c8e4f8a')
        const ticketId = reference.substring(
          'ticket-'.length
        )
        ...
        if (!ticket) {
          console.error(
            'Paystack Webhook: Ticket not found for ID extracted from reference:',
            ticketId
          )
          // Acknowledge receipt to Paystack even if we can't find the record.
          return res.sendStatus(200)
        ...
        if (ticket.payment_status === 'completed') {
          console.log(
            'Paystack Webhook: Ticket payment already completed for reference:',
            reference
          )
          return res.sendStatus(200) // Already processed, acknowledge receipt
        }
        ...
        await ticket.save()
        console.log(
          'Paystack Webhook: Marked ticket as completed:',
          reference
        )

        // Increment the tickets_sold count on the associated TicketType
        const ticketType =
          await TicketType.findByIdAndUpdate(
            ticket.ticket_type, // Use the ticket_type ObjectId from the ticket
            { $inc: { tickets_sold: 1 } }, // Increment by 1 for this single ticket purchase
            { new: true } // Return the updated TicketType document
          )

        if (!ticketType) {
          console.error(
            'Paystack Webhook: TicketType not found for ticket:',
            ticket._id
          )
          ...
          console.log(
            `Paystack Webhook: Updated ticket type "${ticketType.name}" sold count: +1. New total sold: ${ticketType.tickets_sold}`
          )
        }
      } else {
        // --- Handle Unrecognized Reference Format ---
        console.warn(
          'Paystack Webhook: Received charge.success event with unhandled reference format:',
          reference
        )
    ...
    const reference = event.data.reference
    console.log(
      'Paystack Webhook: Processing charge.failed event for reference:',
      reference
    )
    ...
    try {
      ...
      if (reference && reference.startsWith('vote-')) {
        ...
        if (vote) {
          ...
          await vote.save()
          console.log(
            'Paystack Webhook: Marked vote as failed:',
            reference
          )
        } else {
          console.warn(
            'Paystack Webhook: Failed vote reference not found in DB:',
            reference
          )
        ...
        if (ticket) {
          ...
          await ticket.save()
          console.log(
            'Paystack Webhook: Marked ticket payment as failed:',
            reference
          )
        } else {
          console.warn(
            'Paystack Webhook: Failed ticket reference not found in DB:',
            reference
          )
          // Acknowledge receipt even if not found.
        }
        ...
        console.warn(
          'Paystack Webhook: Received charge.failed event with unhandled reference format:',
          reference
        )
        // Acknowledge receipt.
      }
      ...
      console.error(
        'Paystack Webhook: Error processing charge.failed event:',
        err
      )
